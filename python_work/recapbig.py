...
# We’ve used integers which is just whole numbers. This is the mathmatical value one hundred and twenty three
print(123)
...
# We looked at methods we can do to our strings – we can use len to count how many characters there are in the string

print(len("Hello"))

# shows us "5" in the terminal - line 18 is asking us to print the length of the string
...
print("the quick brown fox".strip("the quick" "fox")) #use can use multiple strings if they target the start and the end
...
# * is multiplication
# ** is to the power of
# % is modulous - it divides two numbers - but it only returns the remainder
...
# else is out last statement - it's a cover-all. If nothing mathces, else will run. That's why it doesn't need a condition
# ALWAYS be aware of your orders in if statements. Python reads top down, and as soon as it hits a statement that's true, it will run it, even if there's
# a more true statement after!
...
# at this point though, how can we get our code to run again and again?

#--------------------------------------------------------------------------------------------------------------------------------------------#

#Day 4 - Functions

# functions help us break our code down into useable chunks
# they ONLY run when we ask them to

# we've used functions before - print is a function! But how do we write our own?

# def function_name()

# def is the keyword - it DEFINES our function
# we should name our function sensibly - let's stick with our bank

def cash_withdrawal():
    print("You are withdrawing cash")
    print("That must be nice!")

# So far, all we've done is define what we want the function to do when we call it
# we need to actually call it!
# we call our functions using their name

cash_withdrawal()

# inside those brackets at the end, we can put parameters
# parameters are like variables - but we can pass them through the function call

# when we define our function, we need to say we'll be using parameters
...
# when we run this function, every instance of "amount" or "accnum" now has the value we gave it when we called the function - so amount is 300 and accnum is 12345678
cash_withdraw(300,12345678) 
cash_withdraw(600,87654321) 
cash_withdraw(10,11882277) 

#we can call this function AS MANY times as we like - and each time we do, those paramaters change

# things a function only exist in that function - unless we tell them to!

# all the variables we've made so far are global - they exist everywhere
# if we make a variable in a function - it's local only to that function
# #unless we tell it otherwise!
# we must make our variable global!
# this is the same if we want to update a variable within a function
# so let's try our pin tries again
...
#See how tries updates, so when we get it wrong the 4th time we're locked out? If we put tries INSIDE the function, it would update the value of the variable within the function - so everytime the function ran, tries would be back to 0. If we didn't use global, we couldn't UPDATE the value of the variable. This allows us to access variables and variable updates outside of a local unction - be that globally or within another function!

#Something else useful is return. Return updates/stores a value, and returns that value back to the caller.
#for example
...
#rather than asking the function itself to print the answer, or make a variable with the answer, we can store that value "10" and return that value in place of the function to the function caller. In this case, the function caller is within our print.

#this can be really efficient - it saves us having to make other variables to save answers in, or it can save us from having to ask our function to something we don't want it to. I might not ALWAYS want my function to print the added value - so I wouldn't write that in to my function. But in this specific case, I do want to see the result.

# Return lets me do that! It also lets functions work with eachother

def multiply_by_nine_fifths(celsius):  #this is defining the function x by 9/5 - it takes one parameter - celsius
    return celsius * (9/5)             # the action o the function is to return this value - in this case - 15 x 9/5 - which is 27

def get_fahrenheit(celsius):           #this is defining the function get_fahrenheit - it takes one parameter - celsius
    return multiply_by_nine_fifths(celsius) + 32   # the action of the function is to return the returned value of x by 9/5, and add 32 to it

print("The temperature is {}°F".format(get_fahrenheit(15))) # this is the caller - it calls the function - which then calls the function

# so

# print("The temperature is {}°F".format(get_fahrenheit(15))) # this is the caller - it is the first thing done. It calls get farenheit

# get_fahrenheit(celsius):           
#     return multiply_by_nine_fifths(celsius) + 32    # get_fah is then the caller for x by 9/5 - so it moves to that function

# multiply_by_nine_fifths(celsius):  
#     return celsius * (9/5)             x by 9/5 then occurs - at updates itself to have the value of the sum - 27

# we then go back to get_fah - that is the function we first called! It just happened to call another function for its completion!
# 
# get_fah now reads - return 27 + 32
# so get_fah is then 59 - it returns that value to our intital print statement - so for all intents and purposes, the print statement is print(59)

#Output: The temperature is 59°F

#Let's take a break from functions =p
